    ...
    try {
        ...
        const getData = (value, field, valueSearch) => {
            fetch('/db/db.json')
                .then((res) => res.json())
                .then((data) => {
                    const array = valueSearch !== null
                        ? data.filter((good) => good.name.toLowerCase().includes(valueSearch.toLowerCase()))
        ...
        links.forEach((link) => {
            link.addEventListener('click', (event) => {
                ...
                getData(linkValue, linkDataField, null);
            });
        });
        ...
        buttonSearch.addEventListener('click', () => {
            getData(null, null, inputSearch.value);
        });
